...
{  
    ...
    struct bbf_data* bbf_data1; //bbf_data是一个用来存放临时特征数据和特征间距离的缓存结构  
    ...
    _nbrs = (feature**)calloc( k, sizeof( struct feature* ) ); //给查找结果分配相应大小的内存  
    ...
    {  
        ...
        {  
            ...
            bbf_data1 = (bbf_data *)malloc( sizeof( struct bbf_data ) );//新建bbf结构  
            if( ! bbf_data1 )  
            {   //出错处理  
                fprintf( stderr, "Warning: unable to allocate memory," " %s line %d\n", __FILE__, __LINE__ );  
                goto fail;  
            }  
            bbf_data1->old_data = tree_feat->feature_data;//保存第i个特征点的feature_data域以前的值  
            bbf_data1->d = descr_dist_sq(feat, tree_feat);//当前搜索点和目标点之间的欧氏距离  
            tree_feat->feature_data = bbf_data1;//将bbf结构赋给此特征点的feature_data域  
    ...
    {  
        ...
        _nbrs[i]->feature_data = bbf_data1->old_data;//将之前的数据赋值给feature_data域  
        free( bbf_data1 );  
    }  
    *nbrs = _nbrs;  
    return n;  
...
{  
    ...
    kd_node1 =(kd_node *)malloc(sizeof(struct kd_node ) );//分配内存  
    memset( kd_node1, 0, sizeof( struct kd_node ) );  
    kd_node1->ki = -1;//枢轴索引  
    kd_node1->features = features;//节点对应的特征点集  
    kd_node1->n = n;//特征点的个数  
